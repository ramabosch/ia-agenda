...
def render_conversation_page():
    ...
    with st.expander("Ejemplos de consultas que podes hacer"):
        ...
        st.write("- que tengo hoy con Cam")
        st.write("- resumime Cam y decime que haria primero")
        st.write("- que vence y que me preocuparia")
        st.write("- que tengo urgente y atrasado")
        st.write("- comentame dashboard y despues decime que sigue")
        st.write("- que tengo para manana y que esta bloqueado")
        st.write("- que me preocuparia de Cam y que le diria al cliente")
        st.write("- que haria ahora con esto y que vence esta semana")
        st.write("- resumime el dashboard de ventas, no el otro")
        st.write("- que me preocuparia del proyecto de automatizacion de Cam")
        st.write("- agrega una tarea al proyecto de dashboard comercial")
        st.write("- comentame el proyecto de onboarding de Cam")
        st.write("- en dashboard comercial")
        st.write("- el de ventas")
        st.write("- no el otro")
        st.write("- que hiciste recien")
        st.write("- que resolviste")
        st.write("- por que elegiste esa")
        st.write("- que quedo bloqueado por seguridad")
        st.write("- que parte entendiste")
        st.write("- que accion ejecutaste")
    ...
    if st.button("Consultar"):
        ...
        with st.expander("Debug sesiones 20/21/22/22.1/23/25/26"):
            ...
            st.write("Expected scope:", parsed_query.get("_expected_scope"))
            st.write("Secondary descriptor:", parsed_query.get("_secondary_descriptor"))
            st.write("Used previous candidates:", parsed_query.get("_used_previous_candidates", False))
            ...
            st.write("Sub-intent principal")
            st.write(parsed_query.get("_compound_primary_intent"))
            st.write("Sub-intent secundario")
            st.write(parsed_query.get("_compound_secondary_intent"))
            st.write("Partes resueltas")
            st.json(parsed_query.get("_compound_resolved_parts", []))
            st.write("Partes degradadas")
            st.json(parsed_query.get("_compound_degraded_parts", []))
            st.write("Reutilizo snapshot dentro del mismo turno")
            st.write(parsed_query.get("_compound_reused_snapshot", False))
            st.write("Pidio clarificacion parcial")
            st.write(parsed_query.get("_compound_partial_clarification", False))
            st.write("Foco de auditoria")
            st.write(parsed_query.get("_audit_focus"))
            st.write("Fuente de auditoria")
            st.write(parsed_query.get("_audit_source"))
            st.write("Traza operativa")
            st.json(parsed_query.get("_audit_trace", {}))
